<template>
  ...
  <div v-if="resendMode && selectedUserMessage" :class="compact ? 'py-3' : 'py-4'">
    ...
    <div class="mb-2 flex items-center gap-1.5 flex-wrap">
      <button
        ...
        :class="{
          'hover:opacity-80': canEnableWebSearch,
          'cursor-pointer opacity-50 hover:opacity-70': !canEnableWebSearch,
        }"
    ...
    <div
      class="rounded-lg"
      :class="compact ? 'mb-2 p-3' : 'mb-3 p-4'"
    ...
    <div class="flex justify-end">
      <button
        ...
        :style="
          selectedModelIds.length === 0 || isSending
        ...
        :title="
          isSending
            ...
            : selectedModelIds.length > 1
              ? `Resend to ${selectedModelIds.length} models`
              : 'Resend message'
        "
      >
  ...
  <div v-else>
    ...
    <div
      v-if="branchContextText"
      class="bg-accent/10 border border-accent/30 rounded-lg animate-slide-in"
      :class="compact ? 'mb-3 p-3' : 'mb-4 p-3.5'"
    >
      ...
      <div
        class="italic pl-3 border-l-2 border-accent/50 max-h-24 overflow-y-auto"
    ...
    <div class="flex items-end" :class="compact ? 'gap-2' : 'gap-3'">
      ...
      <button
        ...
        class="flex items-center justify-center rounded-lg transition-all"
        :style="
          !inputText.trim() || !canSend || selectedModelIds.length === 0 || isSending
        ...
        :title="
          isSending
            ? 'Sending...'
            : selectedModelIds.length > 1
              ? `Send to ${selectedModelIds.length} models`
              : 'Send message'
        "
      >
        <div v-if="isSending" class="loading-spinner-small"></div>
